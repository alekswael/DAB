...
def parse_arguments():
    ...
    parser.add_argument("--only_docs",
                        dest="only_docs",
                        default=None,
                        nargs="*",
                        help="list of document identifiers on which to focus the evaluation " + "(if not specified, computes the evaluation measures for all documents)"
                        )
...
class GoldCorpus:
    ...
    def __init__(self, gold_standard_json_file:str, spacy_model = "en_core_web_md"):
        ...
        fd.close()
        print("Reading annotated corpus with %i documents"%len(data_list))
        ...
        for entry_dict in data_list:
            ...
            new_doc = GoldDocument(doc_id=int(entry_dict["id"]),
        ...
        for annotation_dict in annotations:
            ...
            for entity in self._get_entities_from_mentions(annotation_dict["result"]):
                ...
                if entity.entity_id in self.entities:
                    ...
                    # TODO: "entity_id" is "id" in my annotation_result_dict
            ...
            if result_dict["type"]=="labels": # Exclude the relation_dicts
                ...
                if result_dict["value"]["labels"][0] in ["DIREKTE", "KVASI"]:
                    ...
                    if result_dict["value"]["labels"][0] not in ["DIREKTE", "KVASI"]:
                        raise RuntimeError("Unspecified or invalid identifier type: %s"%(result_dict["identifier_type"]))

                    need_masking = result_dict["value"]["labels"][0] in ["DIREKTE", "KVASI"]
                    ...
                    # Otherwise, we create a new entity with one single mention
                        ...
                        new_entity = AnnotatedEntity(entity_id=entity_id,
                                                    mentions=[(start, end)],
...
def get_masked_docs_from_file(masked_output_file:str):
    ...
    for doc_id, masked_spans in masked_output_docs.items():
        doc = MaskedDocument(int(doc_id), []) # Create a empty masked document class per doc
        ...
        for start, end in masked_spans:
            doc.masked_spans.append((start, end)) # Append the masked spans to the MaskedDocument object
